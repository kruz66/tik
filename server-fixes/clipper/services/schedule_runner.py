...
def process_queue_items(
    ...
    *,
    ...
    # IDs in this processing batch must not be treated as "already seen" solely
    # because a prior step persisted them into seen_video_ids.
    batch_ids = {
        str(i.source_video_id or "").strip()
        for i in items
        if i.status == ScheduledUploadQueue.Status.PENDING and str(i.source_video_id or "").strip()
    }
    for item in items:
        if quota_hit:
            break
        ...
        if item.user_id not in getattr(process_queue_items, "_known_cache", {}):
            ...
            pass
        if not known_ids:
            known_ids = _known_video_ids_for_user(item.user) - batch_ids
    ...
    *,
    ...
    try:
        ...
        if created and queue_item.source:
            ...
            _remember_seen_ids(queue_item.source, [str(queue_item.source_video_id)])
...
def check_scheduled_source(source: ScheduledSource) -> dict:
    ...
    # Remember already-listed (non-new) IDs so older clips never become "new" later.
    # Do NOT mark brand-new IDs as seen yet — process_queue_items consults
    # seen_video_ids and would immediately skip them as "already seen".
    new_id_set = {
        str(v.get("id") or "").strip() for v in new_videos if str(v.get("id") or "").strip()
    }
    _remember_seen_ids(
        source,
        [vid for vid in fetched_ids if vid and vid not in new_id_set],
    )
    ...
    if ready:
        ...
        # Persist IDs that completed so future watches do not re-queue them.
        completed_ids = list(
            ScheduledUploadQueue.objects.filter(
                id__in=[row.id for row in to_process],
                status=ScheduledUploadQueue.Status.COMPLETED,
            ).values_list("source_video_id", flat=True)
        )
        if completed_ids:
            _remember_seen_ids(source, [str(x) for x in completed_ids if x])
        if uploaded:
