...
def _search_videos_duckduckgo(handle: str, *, max_urls: int) -> list[str]:
    ...
    for query in queries:
        ...
        try:
            ...
            logger.exception("DuckDuckGo lookup failed for TikTok alias %s", handle)
            continue
...
def _search_tiktok_video_urls_for_username(username: str, *, max_urls: int = 8) -> list[str]:
    ...
    # Prefer fast sources first; Wayback is reliable from datacenter IPs.
    for searcher in (
        ...
        try:
            ...
            logger.exception("Video URL searcher %s failed for @%s", searcher.__name__, handle)
            continue
        if found:
            logger.info(
